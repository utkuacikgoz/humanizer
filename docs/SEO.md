...
**Progress (owner-reported 2026-08-25):** Google Search Console is connected, `sitemap.xml` is submitted, and
Bing Webmaster Tools verification is complete. The production deploy has since run, so the live host is serving
the build that carries the `msvalidate.01` token, the corrected robots directives, and the `lastmod` values.
Steps 1 to 4 and the Bing step are therefore done. Continue from step 5, the live URL Inspection of `/`,
`/privacy`, and `/terms`.

Nothing above was verified from this repository: outbound access to `ownword.pro` is blocked from the agent
sandbox, so every live-site statement here is the owner's report.
