...
Decision: The canonical customer-facing identity is Ownword at `ownword.pro`. It comes from a typed configuration object containing `productName`, `productTagline`, `domain`, nullable `supportEmail`, nullable `legalCompanyName`, and `socialHandles`. Use `humanizer` only as the internal codename or a generic search-category term.

Reason: The founder confirmed the brand and domain on 2026-08-24. The legal operator, monitored support mailbox, social profiles, and official logo are not confirmed.

Consequence: Public UI, metadata, structured data, legal drafts, and documentation use Ownword consistently. Unverified identity fields stay unset. Use a text wordmark and no custom favicon until approved brand assets exist. Tests detect stale display identity and placeholder business claims.
...
### D-013 — M2 billing is built while some business and Stripe details remain pending

Decision: Product Orchestrator directed engineering to proceed with the M2 payment and entitlement backlog before every commercial detail was available. Ownword and `ownword.pro` are now confirmed. The legal company name, monitored support address, official visual assets, and live Stripe credentials remain pending. Values stay centralized through `productConfig`, `STRIPE_*` environment variables, and the versioned Stripe price mapping. Missing required payment configuration must fail closed. Draft legal pages remain clearly labeled and out of search until Legal approves the operator, contact route, and terms.
